defmodule Pky.Models.MioriUserData do
  ...
  defmodule Discord do
    ...
    @derive {Jason.Encoder,
             only: [:uuid, :global_username, :avatar_url, :banner_url, :created_at, :activities]}
    embedded_schema do
      field(:uuid, :integer)
  ...
  defmodule Anilist do
    ...
    @primary_key false
    @derive {Jason.Encoder,
             only: [:id, :name, :site_url, :avatar_url, :banner_url, :statistics, :activities]}
    embedded_schema do
      field(:id, :integer)
      field(:name, :string)
    ...
    defmodule Statistics do
      ...
      defmodule AnimeStats do
        ...
        def changeset(s, p),
          do: cast(s, p, [:count, :mean_score, :episodes_watched, :minutes_watched])
      end
    ...
    defmodule Activity do
      ...
      embedded_schema do
        ...
        embeds_one(:messenger, __MODULE__.User)
        embeds_one(:recipient, __MODULE__.User)
        field(:status, :string)
        field(:progress, :string)
        embeds_one(:media, __MODULE__.Media)
        field(:text, :string)
        embeds_one(:user, __MODULE__.User)
        embeds_many(:likes, __MODULE__.User)
        embeds_many(:replies, __MODULE__.Reply)
      end

      def changeset(schema, params) do
        schema
        |> cast(params, [:id, :type, :url, :created_at_utc, :message, :status, :progress, :text])
        |> cast_embed(:messenger)
      ...
      defmodule User do
        ...
        def changeset(s, p), do: cast(s, p, [:id, :name, :avatar_url])
      end
      ...
      defmodule Reply do
        ...
        import Ecto.Changeset
        @primary_key false
        @derive {Jason.Encoder, only: [:id, :text, :created_at, :user]}
        embedded_schema do
          field(:id, :integer)
          field(:text, :string)
          field(:created_at, :integer)
          embeds_one(:user, Pky.Models.MioriUserData.Anilist.Activity.User)
        end

        def changeset(s, p), do: cast(s, p, [:id, :text, :created_at]) |> cast_embed(:user)
      end
    end
  end
  ...
  defmodule Spotify do
    ...
    @primary_key false
    @derive {Jason.Encoder,
             only: [:display_name, :profile_url, :avatar_url, :recently_played, :user_playlists]}
    embedded_schema do
  ...
  defmodule Osu do
    ...
    @primary_key false
    @derive {Jason.Encoder,
             only: [:id, :username, :avatar_url, :cover_url, :join_date, :cover, :recent_scores]}
    embedded_schema do
      field(:id, :integer)
    ...
    defmodule Score do
      ...
      defmodule Statistics do
        ...
        @primary_key false
        @derive {Jason.Encoder,
                 only: [:count_300, :count_100, :count_50, :count_miss, :count_katu, :count_geki]}
        embedded_schema do
        ...
        def changeset(s, p),
          do:
            cast(s, p, [:count_300, :count_100, :count_50, :count_miss, :count_katu, :count_geki])
      end
      ...
      defmodule BeatmapSet do
        ...
        @primary_key false
        @derive {Jason.Encoder,
                 only: [:id, :title, :artist, :creator, :status, :preview_url, :covers]}
        embedded_schema do
          field(:id, :integer)
          field(:title, :string)
          field(:artist, :string)
        ...
        def changeset(s, p),
          do:
            cast(s, p, [:id, :title, :artist, :creator, :status, :preview_url])
            |> cast_embed(:covers)
